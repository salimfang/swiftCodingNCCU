...
func distinctAuthors() -> Set<String> {
    var authorSet = Array<String>()
    
    for bookSelected in books{
        authorSet.append(bookSelected["author"]!)
    }
    
    return(Set(authorSet))
}
...
func totalBookPrice() -> Double {
    ...
    for bookSelected in books{
        totalPrice = Double(bookSelected["price"]!)! + totalPrice
    }
    return(totalPrice)
}
...
func getBook(at index: Int) -> (title: String, author: String, price: Double)?
{
    if index > books.count-1{
        ...
        let bookDetail = books[index]
        return (title: bookDetail["title"]!, author: bookDetail["author"]!, price: Double(bookDetail["price"]!)!)
    }
}
